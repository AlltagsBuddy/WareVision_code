# WareVision – Dokumentation

**Warenwirtschafts- und Werkstattsystem**

---

## Dokumente

| Dokument | Beschreibung |
|----------|--------------|
| [ANLEITUNG_PROJEKTSTRUKTUR.md](./ANLEITUNG_PROJEKTSTRUKTUR.md) | **Schritt-für-Schritt: Projektstruktur erstellen** |
| [CODING_GUIDELINES.md](./CODING_GUIDELINES.md) | Coding-Standards, Commits, Namenskonventionen |
| [ROADMAP.md](./ROADMAP.md) | Haupt-Roadmap mit Phasen, Epics, Zeitplan |
| [SPRINT_ROADMAP.md](./SPRINT_ROADMAP.md) | Detaillierte Sprint- und Epic-Story-Matrix |
| [LASTENHEFT.md](./LASTENHEFT.md) | Referenz zum Lastenheft (Version 1.0) |

---

## Projektstruktur

```
WareVision_code/
├── frontend/          # React + TypeScript
├── backend/           # FastAPI (Python)
├── infra/             # Docker, CI/CD, Infrastruktur
└── docs/              # Dokumentation
```

---

## Quick Start

1. **Repository klonen**
2. **Branching:** `main` (Produktion), `develop` (Entwicklung), `feature/*` (Features)
3. **Commits:** Conventional Commits (z. B. `feat: add customer CRUD`)

---

*WareVision – Version 1.0*
